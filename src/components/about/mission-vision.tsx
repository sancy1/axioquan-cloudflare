...
import { useEffect, useState, useRef } from 'react';
import { motion, useInView } from 'framer-motion';
import { Target, Eye, Brain, Zap, Cpu, CircuitBoard, Globe, Rocket, Shield } from 'lucide-react';
...
export default function MissionVision() {
  ...
  const items = [
    { icon: <Target className="h-8 w-8" />, title: 'Our Mission', description: 'To democratize quality education by making it accessible, engaging, and effective for everyone, everywhere.', gradient: 'from-blue-500 to-cyan-500', iconBg: 'from-blue-500 to-cyan-500' },
    { icon: <Eye className="h-8 w-8" />, title: 'Our Vision', description: 'A world where anyone can learn anything, anytime, with personalized guidance and community support.', gradient: 'from-violet-500 to-pink-500', iconBg: 'from-violet-500 to-pink-500' },
    { icon: <Brain className="h-8 w-8" />, title: 'Our Approach', description: 'Blending cutting-edge technology with pedagogical excellence to create transformative learning experiences.', gradient: 'from-emerald-500 to-teal-500', iconBg: 'from-emerald-500 to-teal-500' },
    { icon: <Zap className="h-8 w-8" />, title: 'Our Promise', description: 'Delivering measurable skill development and career advancement through practical, project-based learning.', gradient: 'from-amber-500 to-orange-500', iconBg: 'from-amber-500 to-orange-500' },
  ];
  ...
  const floatingIcons = [
    { Icon: Cpu, delay: 0, duration: 20, left: '5%', top: '10%' },
    { Icon: CircuitBoard, delay: 2, duration: 25, left: '90%', top: '15%' },
    { Icon: Globe, delay: 4, duration: 22, left: '10%', top: '80%' },
    { Icon: Rocket, delay: 1, duration: 18, left: '85%', top: '70%' },
    { Icon: Shield, delay: 3, duration: 24, left: '45%', top: '5%' },
  ];

  return (
    /* overflow-hidden removed so grid reaches all edges */
    <section ref={sectionRef} className="relative px-4 sm:px-6 lg:px-8 py-24 bg-[#0a0a12]">

      {/* ── Grid + decorative layer — position:absolute fills section exactly ── */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">

        {/* Fine 48px indigo grid — no offset, touches all edges */}
        <div
          ...
          style={{
            ...
            backgroundPosition: '0 0',   /* anchored to top-left corner */
          }}
        />

        {/* Bold 192px accent grid overlay for depth */}
        <div
          className="absolute inset-0 opacity-[0.04]"
          style={{
            backgroundImage: `
              linear-gradient(rgba(99,102,241,1) 2px, transparent 2px),
              linear-gradient(90deg, rgba(99,102,241,1) 2px, transparent 2px)
            `,
        ...
        {/* Indigo glow orbs */}
        <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-indigo-600/8 rounded-full blur-3xl" />
        <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-blue-600/8 rounded-full blur-3xl" />
        ...
        {floatingIcons.map((item, i) => (
          ...
          >
            <item.Icon className="w-8 h-8 text-indigo-400/10" />
          </motion.div>
        ))}
        ...
        <motion.div
          ...
          className="absolute left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-indigo-400/20 to-transparent"
        />
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {/* Heading — "What Drives" is dark/black so it reads on the light sections above */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        ...
        >
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6">
            {/* ← changed from text-white to text-gray-900 so it's visible */}
            <span className="text-gray-400">What Drives</span>
            <span className="bg-gradient-to-r from-blue-400 via-violet-400 to-pink-400 bg-clip-text text-transparent"> AxioQuan</span>
          </h2>
          <p className="text-lg text-gray-400 max-w-3xl mx-auto">
            We're more than just an e-learning platform. We're a movement dedicated to transforming how the world learns and grows.
          </p>
        </motion.div>

        {/* Cards — dark bg + inner grid + thick visible border */}
        <motion.div
          variants={containerVariants}
          initial="hidden"
          animate={isInView ? 'visible' : 'hidden'}
        ...
        >
          {items.map((item, index) => (
            ...
            >
              {/* Hover glow */}
              <div className={`absolute -inset-0.5 bg-gradient-to-r ${item.gradient} opacity-0 group-hover:opacity-40 rounded-2xl blur-xl transition-all duration-500`} />

              {/* Card — dark base with its own inner grid + thick border */}
              <div
                className="relative rounded-2xl p-6 overflow-hidden transition-all duration-300"
                style={{
                  /* Dark card background */
                  backgroundColor: '#0d0d1a',
                  /* Thick 2px indigo border — clearly visible */
                  border: '2px solid rgba(99,102,241,0.35)',
                  /* Inner grid same pattern as section but slightly brighter */
                  backgroundImage: `
              ...
              >
                {/* Solid dark layer so inner grid doesn't bleed through content */}
                <div className="absolute inset-0 bg-[#0d0d1a]/80 rounded-2xl" />

                {/* Hover border brightening */}
                <div className={`absolute inset-0 rounded-2xl border-2 border-transparent group-hover:border-indigo-500/50 transition-colors duration-300`} />

                <div className="relative z-10">
                  {/* Icon */}
                  <div className="relative mb-6">
                    <div className={`w-16 h-16 rounded-xl bg-gradient-to-r ${item.iconBg} flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                      {item.icon}
                    </div>
                  </div>
                  ...
                  {/* Description — light gray on dark card */}
