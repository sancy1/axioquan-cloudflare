...
// src/components/messaging/conversation-list.tsx
// Left sidebar — shows all conversations with unread badges
// Phase 4B: Directory button opens DirectoryModal
...
export default function ConversationList({
  ...
  return (
    <>
      ...
      {/* Footer — Directory button opens modal */}
      <div className={`
        px-4 py-3 border-t ${theme.border}
        flex items-center justify-between
      `}>
        ...
        {/* Directory button — now opens modal */}
        <button
          onClick={() => setShowDirectory(true)}
          className={`
            flex items-center gap-1 text-xs font-medium
            px-2.5 py-1.5 rounded-lg border ${theme.border}
            ${theme.textSec} hover:${theme.text}
            hover:border-[#4f6ef7]/40 transition-all
          `}
        >
          <Users className="w-3 h-3" />
          Directory
