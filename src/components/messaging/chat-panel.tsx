...
// src/components/messaging/chat-panel.tsx
// Phase 4C: Conversation Details panel (slide-out) + Manage Participants modal
...
import {
  ArrowLeft, MoreHorizontal, Bell, Paperclip,
  Smile, Send, Users, Trash2, X, Settings
} from 'lucide-react'
import { formatDistanceToNow } from 'date-fns'
import EmojiPicker from './emoji-picker'
...
interface ChatPanelProps {
  ...
  onBack:                 () => void
  onConversationDeleted?: (id: string) => void
  onConversationLeft?:    (id: string) => void
}
...
export default function ChatPanel({
  ...
  const canManage = currentUserRole === 'instructor' || currentUserRole === 'admin'
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const payload = (e as CustomEvent).detail as WsMessagePayload
      if (payload.conversationId === conversation.id) fetchMessages()
    }
    window.addEventListener('messaging:new-message', handler)
    return () => window.removeEventListener('messaging:new-message', handler)
  }, [conversation.id, fetchMessages])
  ...
  // ── Mark as read on textarea focus ────────────────────────────────────────
  const handleTextareaFocus = useCallback(async () => {
    try {
      await fetch('/api/messaging/proxy/notifications/read-all', {
        method:  'PATCH',
  ...
  const handleEmojiSelect = (emoji: string) => {
    const textarea = textareaRef.current
    if (!textarea) { setContent((prev) => prev + emoji); return }
    const start      = textarea.selectionStart
    const end        = textarea.selectionEnd
  ...
  const handleClearConversation = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) onConversationDeleted?.(conversation.id)
    } catch (err) {
      console.error('Failed to delete conversation:', err)
    } finally {
      setClearing(false)
  ...
  return (
    <div className="flex flex-1 min-w-0 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        `}>
          ...
          <div className="relative flex-shrink-0">
            {isGroup ? (
              ...
              <img src={other.image} alt={name} className="w-9 h-9 rounded-full object-cover" />
            ) : (
              <div className={`w-9 h-9 rounded-full ${getAvatarColor(name)} flex items-center justify-center`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <div className={`text-sm font-semibold ${theme.text} truncate`}>{name}</div>
              {isGroup && (
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-purple-500/15 text-purple-400 flex-shrink-0 flex items-center gap-1">
                  <Users className="w-2.5 h-2.5" />
                  Group
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className={`
                p-1.5 rounded-lg border ${theme.border}
                ${theme.textSec} transition-colors
                ${showDetailsPanel ? theme.accentSoft : ''}
              `}
            ...
            {/* Bell */}
            {/* <button className={`p-1.5 rounded-lg border ${theme.border} ${theme.textSec} transition-colors`}>
              <Bell className="w-3.5 h-3.5" />
            </button> */}
            ...
            <div className="relative" ref={menuRef}>
              ...
              {showMenu && (
                <div className={`
                  absolute right-0 top-full mt-1 w-52 rounded-xl
                  border ${theme.border} ${theme.surface}
                  shadow-2xl overflow-hidden z-50
                `}>
                  {/* Manage Participants — instructor/admin only */}
                  {canManage && (
                    <button
                      onClick={() => { setShowMenu(false); setShowManageModal(true) }}
                      className={`
                        w-full flex items-center gap-2.5 px-4 py-2.5
                        text-xs ${theme.textSec} hover:${theme.text}
                        hover:${theme.surface2} transition-colors text-left
                      `}
                    >
        ...
        {showClearConfirm && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className={`w-80 rounded-2xl border ${theme.border} ${theme.surface} p-6 shadow-2xl`}>
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-full bg-red-500/15 flex items-center justify-center">
                  <Trash2 className="w-5 h-5 text-red-400" />
                </div>
                <div>
                  <div className={`text-sm font-semibold ${theme.text}`}>Delete conversation</div>
                  <div className={`text-xs ${theme.textMuted}`}>This cannot be undone</div>
                </div>
              </div>
              <p className={`text-xs ${theme.textSec} mb-5 leading-relaxed`}>
                Are you sure you want to delete this conversation with{' '}
                <strong className={theme.text}>{name}</strong>? All messages will be permanently removed.
              </p>
              <div className="flex gap-2">
                <button
                  onClick={() => setShowClearConfirm(false)}
                  className={`flex-1 px-4 py-2 rounded-lg text-xs font-medium border ${theme.border} ${theme.textSec} transition-colors`}
                >
                  Cancel
                </button>
                <button
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6 space-y-4 scrollbar-thin">
          {loading ? (
            <div className="flex items-center justify-center h-full">
              <div className={`w-6 h-6 rounded-full border-2 border-t-transparent ${theme.accentBorder} animate-spin`} />
            </div>
          ) : messages.length === 0 ? (
            ...
            <EmptyState
              ...
              participants={
                conversation.otherParticipant
                  ? `${conversation.otherParticipant.name} · 2 participants`
                  : undefined
              }
            />
            ...
            Object.entries(groupedMessages).map(([date, msgs]) => (
              <div key={date}>
                <div className="flex items-center gap-3 my-4">
                  <div className={`flex-1 h-px bg-current opacity-10 ${theme.textMuted}`} />
                  <span className={`text-[10px] font-medium tracking-wider ${theme.textMuted}`}>{date}</span>
                  <div className={`flex-1 h-px bg-current opacity-10 ${theme.textMuted}`} />
                </div>
                <div className="space-y-3">
                  {msgs.map((msg) => {
                    ...
                    return (
                      ...
                      >
                        {!isOwn && (
                          <div className="flex-shrink-0 self-end">
                            {msg.sender?.image ? (
                              <img src={msg.sender.image} alt={msg.sender.name} className="w-7 h-7 rounded-full object-cover" />
                            ) : (
                              <div className={`w-7 h-7 rounded-full ${getAvatarColor(msg.sender?.name ?? '')} flex items-center justify-center`}>
                                <span className="text-white text-[10px] font-bold">{getInitials(msg.sender?.name ?? '?')}</span>
                              </div>
                            )}
                          </div>
                        )}
                        <div className={`flex flex-col max-w-[72%] ${isOwn ? 'items-end' : 'items-start'}`}>
                          ...
                          <div className={`flex items-center gap-1.5 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
                            ...
                            <div className={`
                              px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed
                              ${isOwn ? 'bg-[#4f6ef7] text-white rounded-br-sm' : `${theme.surface2} ${theme.text} rounded-bl-sm`}
                              ${isDeleted ? 'italic opacity-40' : ''}
                            `}>
                              {msg.content}
                            </div>
                          ...
                          <div className={`flex items-center gap-1.5 mt-1 ${theme.textMuted}`}>
                            ...
                            {isOwn && !isDeleted && (
                              <span className={`text-[10px] ${msg.isRead ? 'text-blue-400' : msg.isDelivered ? 'text-green-400' : theme.textMuted}`}>
                                {msg.isRead ? '✓✓' : msg.isDelivered ? '✓✓' : '✓'}
                              </span>
                            )}
                          </div>
        ...
        {/* Input */}
        <div className={`px-5 py-4 border-t ${theme.border} ${theme.surface} flex-shrink-0`}>
          <div className={`flex items-end gap-2.5 px-4 py-3 rounded-xl border ${theme.border} ${theme.inputBg} focus-within:border-[#4f6ef7] transition-colors relative`}>
            <button className={`${theme.textMuted} transition-colors flex-shrink-0 pb-0.5`}>
              <Paperclip className="w-4 h-4" />
            </button>
            <textarea
              ...
              rows={2}
              style={{ resize: 'none' }}
              className={`flex-1 bg-transparent text-sm ${theme.text} placeholder:text-[#4a5568] outline-none min-h-[44px] max-h-[160px] overflow-y-auto`}
            />
            <div className="flex items-center gap-1.5 flex-shrink-0 pb-0.5">
              <span className={`text-[10px] ${theme.textMuted}`}>{content.length}/5000</span>
              <div className="relative">
                <button
                  onClick={() => setShowEmoji(!showEmoji)}
                  className={`transition-colors ${showEmoji ? theme.accentText : theme.textMuted}`}
              ...
              <button
                onClick={handleSend}
                disabled={!content.trim() || sending}
                className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${content.trim() && !sending ? 'bg-[#4f6ef7] text-white hover:bg-[#3d5ce6] scale-100' : `${theme.surface2} ${theme.textMuted} scale-95 cursor-not-allowed`}`}
              >
                {sending
                  ? <div className="w-3.5 h-3.5 rounded-full border-2 border-t-transparent border-white animate-spin" />
                  : <Send className="w-3.5 h-3.5" />
      ...
      {/* ── Conversation Details Panel (slide-in from right) ──────────────── */}
      {showDetailsPanel && (
